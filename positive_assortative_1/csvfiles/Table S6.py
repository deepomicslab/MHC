    ...
    if type(record[6])!=float:
        ...
        if record[1]=="All":
            if record[2]==0:#no mhc csvfiles                        
                ...
                #     # p_top_100=[allrigion_P_top_1000[i] for i in indices_top_100]
                #     # pos_top_100=[allpos[i] for i in indices_top_100]
                #     # mean_100_P=np.mean(p_top_100)
                #     # std_100_P=np.std(p_top_100, ddof=1)
                #     # pvalue=round((1-stats.norm.cdf(mean_100_P, all_mean_arr, all_std_arr)),15)
                #     for j in range(len(allrigion_P_top_1000)):
                #         pvalue=round((1-stats.norm.cdf(allrigion_P_top_1000[j], all_mean_arr, all_std_arr)),15)
                #         onerecord=[]
                #         onerecord+=record[1:8]
                #         onerecord+=[allpos[j],allrigion_P_top_1000[j],pvalue]
                #         allrecords.append(onerecord)
                # else:
                #     continue
                print("mhc region",record)
            ...
            if record[2]==0:#no mhc
                ...
                #     pos_top_100=[allpos[i] for i in indices_top_100]              
                #     for j in range(len(allrigion_P_top_1000)):
                #         pvalue=round((1-stats.norm.cdf(allrigion_P_top_1000[j],mean_dict_different_race[record[1]],std_dict_different_race[record[1]])),15)
                #         onerecord=[]
                #         onerecord+=record[1:8]
                #         onerecord+=[allpos[j],allrigion_P_top_1000[j],pvalue]
                #         allrecords.append(onerecord)
                # else:
                #     continue
                print("mhc record in race",record)
    else:
        print("record",record)
        print("record6",record[6])
        onerecord=[]
...
def generatedf(columns,allrecords):
    ...
    dataframe(dictforDF).to_csv("/data2/wangxuedong/mhc_test_data/similarity_region_pythonfiles/csvfiles_95/csv3_top100_without_corr_frompy.csv")
...
dfnew.to_csv("/data2/wangxuedong/mhc_test_data/similarity_region_pythonfiles/csvfiles_95/csv3_top100_with_corr_frompy.csv")
